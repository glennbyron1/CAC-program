...
**Author:** Glenn Byron
**Last Updated:** June 30, 2026 (v1.4 ship — Ansible STIG remediation LAB-DC01 44.95% → 86.7% + Phase 8 ZT completion + cert-audit tool + portfolio refresh)
...
- ✅ **v1.4 shipped (2026-06-30)** — **Ansible STIG remediation on LAB-DC01: 44.95% → 86.7% SCAP** via the community ansible-lockdown Windows-2022-STIG role driven from a WSL2 control node (`Lab-Kit/08-Ansible-STIG/`). Three severity-tagged phases (CAT I → II → III) with snapshot + `win_ping` verification between each. **8 CAT I findings verified closed** in the POA&M (POA-001/002/003/007/009/010/013/015). Scan archives at `Compliance-Reports/After-Ansible/`. Bundled in the same ship: **Phase 8 ZT extension** — all 13 previously-scaffolded scripts in `Lab-Kit/07-ZeroTrust/` promoted to full, idempotent, parse-clean PowerShell modules (work completed 2026-06-29). Phase 8 ZT goes from "8 full + 13 scaffolds" (v1.0) to **21 full scripts** (v1.4). Per-script summary at `Lab-Kit/07-ZeroTrust/CHANGELOG-Phase8.md`. **Defensive cert-audit tool** shipped at `security/scripts/Check-SelfSignedCerts.ps1` with offline-CA thumbprint protection. **Portfolio refresh** — 5 docx files updated with v1.4 milestone callouts + new `Portfolio/CAC-Program-Plain-English-Overview.docx` (non-technical 2-page explainer) + new `Project-Narrative.md` at the repo root (interview prep + future-self orientation). Honest framing on ZT: scripts are "designed → built" (parse-clean implementations); operational execution + Before/After-ZT evidence is not on the roadmap.
...
- ✅ **Nessus Essentials credentialed scan — DONE 2026-06-25** (point-in-time, see POA&M Group 10 / Document Control row 1.3).
...
- ✅ **Full Ansible STIG hardening pass — DONE 2026-06-30 (v1.4 ship)** — Applied to LAB-DC01 via the community ansible-lockdown Windows-2022-STIG role driven from a WSL2 control node (`Lab-Kit/08-Ansible-STIG/`). Three severity-tagged phases (CAT I → II → III) with snapshot + verification between each. Result: **44.95% baseline → 86.7% post-remediation**. Per-CAT closures: CAT I Fail 9→1, CAT II Fail 105→27, CAT III Fail 6→1. Eight CAT I findings verified closed in the POA&M. Scan archives preserved at `Compliance-Reports/After-Ansible/`. The remaining 29 CAT-fails are items left off by design (`disruption_high: false` + `complexity_high: false` safety guards) plus manual AUDIT controls plus four Server-2025-vs-2022 N/A controls.
...
**Status (bundled into v1.4 ship 2026-06-30):**

- ✅ **Phase 8 scaffold completion — DONE 2026-06-29 (shipped in v1.4 2026-06-30)** — took a stronger path than the original "operationalize the 8" plan: implemented all **13 previously-scaffolded scripts** as full, idempotent PowerShell modules with house-style discipline (`#Requires`, comment-based help, `[CmdletBinding(SupportsShouldProcess)]`, `Set-StrictMode`, `Write-Step/OK/Warn/Skip/Fatal` helpers, banner+summary blocks, `-DryRun`/`-WhatIf` preview paths). All 13 files parse-check clean via `[System.Management.Automation.Language.Parser]`. **Phase 8 ZT extension now ships as 21 full scripts** (was 8 full + 13 scaffolds in v1.0). See `Lab-Kit/07-ZeroTrust/CHANGELOG-Phase8.md` for the per-script summary. Scripts are "designed → built" (parse-clean implementations). Operational execution on Lab-DC01 / gateway / endpoints is not on the roadmap.
...
| 8 — Zero Trust Extension | ✅ **v1.4 (2026-06-30):** 21 full ZT scripts (8 v1.0 + 13 scaffold-completion in v1.4) + Demo-Walkthrough-ZT.md + CHANGELOG-Phase8.md. **Plus Ansible STIG remediation module `Lab-Kit/08-Ansible-STIG/` applied to LAB-DC01 — SCAP 44.95% → 86.7%.** |
...
## Recent Wins (2026-06-30 — v1.4 ship)

**Headline:** LAB-DC01 SCAP score climbed **44.95% → 86.7%** via Ansible STIG remediation. 8 CAT I findings verified closed. Phase 8 ZT extension promoted to 21 full scripts. Defensive cert-audit tool shipped. Portfolio refreshed.

- ✅ **Ansible STIG remediation applied to LAB-DC01 — SCAP 44.95% → 86.7%** via the community `ansible-lockdown/Windows-2022-STIG` role driven from a WSL2 control node on the Hyper-V host. Three severity-tagged phases (CAT I → II → III) applied with VM snapshot + `win_ping` verification between each. **8 CAT I findings verified closed** by post-Ansible SCAP scan (POA-001 SV-254352, POA-002 SV-254353, POA-003 SV-254354, POA-007 SV-254374, POA-009 SV-254378, POA-010 SV-254381, POA-013 SV-254467, POA-015 SV-254475). Per-CAT closures: CAT I Fail 9→1 · CAT II Fail 105→27 · CAT III Fail 6→1. New module: [`Lab-Kit/08-Ansible-STIG/`](Lab-Kit/08-Ansible-STIG/). Scan evidence: [`Compliance-Reports/After-Ansible/`](Compliance-Reports/After-Ansible/). Visual evidence: `Screenshots/08a/08b/08c-stig-dc01-*pct.png`.
- ✅ **Compatibility-fix story documented honestly** — four Server-2025 + ansible-core 2.21 patches captured in `Lab-Kit/08-Ansible-STIG/CHANGELOG.md` with the exact one-liners to re-apply if the role is reinstalled (callback rename, `allow_broken_conditionals`, OS-gate regex widened to accept Server 2025, `default()` guards on skipped state-gather tasks). Four controls disabled as Server-2025 N/A (`wn22_00_000090` TPM/wmic, `_000340` PNRP, `_000420`/`_000430` FTP).
- ✅ **Phase 8 Zero Trust extension — 13 scaffolds shipped as full implementations.** Every previously-scaffolded script in `Lab-Kit/07-ZeroTrust/` is now a full, idempotent, parse-clean PowerShell module with house-style discipline (`#Requires`, comment-based help, `[CmdletBinding(SupportsShouldProcess)]`, `Set-StrictMode`, `Write-Step/OK/Warn/Skip/Fatal` helpers, banner + summary blocks, `-DryRun`/`-WhatIf`). Phase 8 ZT goes from **8 full + 13 scaffolds** (v1.0) to **21 full scripts** (v1.4). Coverage spans all ZT pillars: Authorization & Least Privilege (`New-RBACModel` AGDLP, `Deploy-ResourceGateway`), Device Trust (`Set-DeviceComplianceCheck`, `Update-VPN-DeviceAuth`), Continuous & Conditional Access (`Deploy-ConditionalAccess`, `New-RiskPolicy`), Workload Identity (`New-WorkloadCertTemplate`, `Set-ServiceAccountHardening`, `Enable-mTLS`), Network Segmentation (`Convert-VPNToPerApp`), Visibility & Decisioning (`Deploy-SIEM`, `New-DetectionRules` Sigma + MITRE ATT&CK, `Connect-Analytics-To-Policy` loop closer). Per-script summary at `Lab-Kit/07-ZeroTrust/CHANGELOG-Phase8.md`.
- ✅ **Defensive cert-audit tool shipped — [`security/scripts/Check-SelfSignedCerts.ps1`](security/scripts/Check-SelfSignedCerts.ps1).** Scans LocalMachine + CurrentUser stores plus optional remote TLS endpoints, with SHA-1 thumbprint protection for the offline Root CA so the known-good self-signed root is excluded from the unprotected count. Tags `PROTECTED (Offline CA)` / `SELF-SIGNED` / `CA-ISSUED` / `UNREACHABLE` / `NO CERT` / `ERROR`. CSV + pipeline output. Hardened pre-ship: `-Host` → `-Hostname` (PowerShell `$Host` reserved-name fix), proper `BeginConnect`/`EndConnect` pairing + `try/finally` socket disposal, house-style alignment (`#Requires`, `[CmdletBinding()]`, expanded comment-based help, `-OutputPath` defaulting to `$env:TEMP`).
- ✅ **Lab topology updated — Architecture/Lab-Topology.md (ARCH-ICAM-014).** LAB-DC01 added a second NIC at `10.10.10.10` (Lab Internal) for the WSL Ansible reach path. Host-IP conflict (`vEthernet (External)` was holding `10.10.10.10`) resolved via `Remove-NetIPAddress`. The lab segment remains a flat L2 broadcast domain; the change is administrative reachability, not partitioning. Doc explicitly preserves the "honest caveat on partitioning" framing.
- ✅ **POA&M updated — Document Control v1.4 (June 30, 2026).** 8 CAT I closures recorded with verification links; dashboard updated to ~91 closed / ~309 open. POA-017 (AD data files permissions, manual review) correctly remains open as expected. `Lab-Kit/Ansible/` cross-references updated to `Lab-Kit/08-Ansible-STIG/` throughout the living docs.
- ✅ **Portfolio refresh + new plain-English explainer.** All 5 active Portfolio .docx files refreshed with a v1.4 milestone callout on page 1 (Showcase, PM Brief, Lab Build Guide, Federal Upgrade Path, ZT Architecture Reference). New `Portfolio/CAC-Program-Plain-English-Overview.docx` — 2-page non-technical explainer for hiring managers + recruiters.
- ✅ **Legacy `Lab-Kit/Ansible/` folder retired.** Utility playbooks (`ad-health-check.yml`, `cert-expiry-report.yml`) moved to `Lab-Kit/08-Ansible-STIG/utilities/`. Hand-rolled `windows-stig-hardening.yml` preserved as `utilities/windows-stig-hardening_SUPERSEDED.yml` with a "designed → built → superseded with a better tool" pointer to `remediate-stig.yml`.
- ✅ **Honest framing throughout.** STIG remediation: scripts left off by design via `win2022stig_disruption_high: false` + `complexity_high: false` safety guards; 55 manual AUDIT items deferred; the remaining 29 CAT-fails are exactly what those guards exclude. ZT extension: still "designed → built (parse-clean), not yet built → run." This is the federal documentation pattern.
